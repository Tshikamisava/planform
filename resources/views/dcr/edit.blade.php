...
@section('title', 'Edit Design Change Request')
...
<div class="max-w-4xl mx-auto space-y-6 relative pb-12">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-navy-900 tracking-tight">Edit Design Change Request</h1>
            <p class="text-slate-500 text-xs font-bold uppercase tracking-widest opacity-60">Manifest ID: <span class="font-mono text-slate-700">{{ $dcr->dcr_id }}</span></p>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between mb-8 px-4">
        <div class="flex flex-col items-center flex-1 relative z-10">
            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 bg-secondary text-white shadow-lg shadow-orange-500/30" id="step-icon-0">
                1
            </div>
            <span class="text-[10px] mt-2 font-bold uppercase tracking-widest text-secondary" id="step-label-0">Details</span>
            <div class="absolute top-5 left-1/2 w-full h-0.5 -z-10 bg-slate-200">
                <div class="h-full bg-secondary transition-all duration-500 w-0" id="progress-0"></div>
            </div>
        </div>
        <div class="flex flex-col items-center flex-1 relative z-10">
            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 bg-slate-200 text-slate-400" id="step-icon-1">
                2
            </div>
            <span class="text-[10px] mt-2 font-bold uppercase tracking-widest text-slate-400" id="step-label-1">Impact Analysis</span>
            <div class="absolute top-5 left-1/2 w-full h-0.5 -z-10 bg-slate-200">
                <div class="h-full bg-secondary transition-all duration-500 w-0" id="progress-1"></div>
            </div>
        </div>
        <div class="flex flex-col items-center flex-1 relative z-10">
            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 bg-slate-200 text-slate-400" id="step-icon-2">
                3
            </div>
            <span class="text-[10px] mt-2 font-bold uppercase tracking-widest text-slate-400" id="step-label-2">Attachments</span>
        </div>
    </div>

    <!-- Form -->
    <form id="dcr-form" action="{{ route('dcr.update', $dcr) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-8 min-h-[400px]">
            ...
            <div id="step-0" class="space-y-6 animate-fade-in">
                ...
                <div class="space-y-2">
                    ...
                    <input type="hidden" name="reason" id="reason-input" value="{{ old('reason', $dcr->reason_for_change ?? 'Cost Reduction') }}">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea 
                        ...
                        data-required="true"
                    >{{ old('description', $dcr->description) }}</textarea>
            ...
            <div id="step-1" class="space-y-8 animate-fade-in hidden">
                ...
                <div class="p-6 bg-slate-50 rounded-2xl border border-slate-200 space-y-5">
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input 
                                    ...
                                    name="tooling"
                                    value="1"
                                    {{ old('tooling', $dcr->tooling) == '1' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input 
                                    ...
                                    name="tooling"
                                    value="0"
                                    {{ old('tooling', $dcr->tooling) == '0' || !$dcr->tooling ? 'checked' : '' }}
                    ...
                    <div id="tooling-desc-container" class="hidden animate-fade-in pl-1 space-y-2">
                        ...
                        <textarea 
                            ...
                            rows="3"
                        >{{ old('tooling_desc', $dcr->tooling_desc) }}</textarea>
                ...
                <div class="flex items-center gap-4 p-6 border border-slate-200 rounded-2xl bg-white group cursor-pointer hover:border-secondary/30 transition-all">
                    <input 
                        ...
                        value="1"
                        {{ old('inventory_scrap', $dcr->inventory_scrap) ? 'checked' : '' }}
    ...
    <div id="success-modal" class="hidden fixed inset-0 bg-navy-900/90 backdrop-blur-xl z-[100] flex items-center justify-center p-4 animate-fade-in">
        <div class="bg-white rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] p-12 max-w-md w-full text-center border border-slate-100">
            ...
            <h2 class="text-3xl font-extrabold text-navy-900 mb-3 tracking-tight">Update Successful</h2>
            <p class="text-slate-500 text-sm leading-relaxed mb-10">
                Your DCR has been successfully updated and saved.
            </p>
...
document.addEventListener('DOMContentLoaded', function() {
    // Pre-select the reason button for edit mode
    const reasonInput = document.getElementById('reason-input');
    if (reasonInput && reasonInput.value) {
        const reasonBtn = document.querySelector(`[data-reason="${reasonInput.value}"]`);
        if (reasonBtn) {
            selectReason(reasonBtn, reasonInput.value);
        }
    }

    // Show tooling description if tooling is enabled
    const toolingYes = document.querySelector('input[name="tooling"][value="1"]');
    if (toolingYes && toolingYes.checked) {
        toggleToolingDesc(true);
    }
...
function updateStep(newStep) {
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        if (i < newStep) {
            icon.className = 'w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 bg-secondary text-white shadow-lg shadow-orange-500/30';
            icon.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>';
            label.className = 'text-[10px] mt-2 font-bold uppercase tracking-widest text-secondary';
            ...
            icon.className = 'w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 bg-slate-200 text-slate-400';
            icon.textContent = i + 1;
            label.className = 'text-[10px] mt-2 font-bold uppercase tracking-widest text-slate-400';
